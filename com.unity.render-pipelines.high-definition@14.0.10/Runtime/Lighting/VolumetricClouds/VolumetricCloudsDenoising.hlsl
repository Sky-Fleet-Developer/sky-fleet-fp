...
{
    float4x4 mat = _IsPlanarReflection ? _CameraInverseViewProjection_NO : UNITY_MATRIX_I_VP;
    float4x4 viewMat = UNITY_MATRIX_V;
    PositionInputs posInput = GetPositionInput(fullResCoord, _ScreenSize.zw, deviceDepth, mat, viewMat);
    float4 worldPos = float4(posInput.positionWS, positionFlag);
    float4 prevPos = worldPos;
    float4x4 prevMat = _IsPlanarReflection ? _CameraPrevViewProjection_NO : UNITY_MATRIX_PREV_VP;
    float4x4 currMat = _IsPlanarReflection ? _CameraViewProjection_NO : UNITY_MATRIX_UNJITTERED_VP;
    prevMat._m03 = 0;
    prevMat._m23 = 0;
    currMat._m03 = 0;
    currMat._m23 = 0;
    float4 prevClipPos = mul(prevMat, prevPos);
    float4 curClipPos = mul(currMat, worldPos);
