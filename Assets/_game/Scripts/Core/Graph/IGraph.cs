...
namespace Core.Graph
{
    ...
    public static class GraphExtensions
    {
        ...
        {
            if (propertiesContainer != null && propertiesContainer.TryGetProperty(Property.WiresPropertyName, out var wiresProp) && propertiesContainer.TryGetProperty(Property.AutoConnectPowerWirePropertyName, out var acpwProp))
            {
                graph.Init(wiresProp.values.Select(x => x.GetObjectValue<WireConfiguration>()), acpwProp.values[0].intValue == 1);
                return true;
            }
            graph.Init(Enumerable.Empty<WireConfiguration>(), false);
            return false;
        }
    }
