...
namespace Waiters
{
    ...
    public class Lerper
    {
        private UnityEngine.MonoBehaviour owner;
        ...
        public Lerper(UnityEngine.MonoBehaviour owner, Getter<float> getter, Action<float> setter, float duration, float target)
        {
            this.getter = getter;
            this.setter = setter;
    ...
    public class Waiter
    {
        private UnityEngine.MonoBehaviour owner;
        private Coroutine current;
        public Waiter(UnityEngine.MonoBehaviour owner, float delay, Action onComplete)
        {
            this.owner = owner;
            current = null;
            current = owner.StartCoroutine(WaitRoutine(delay, onComplete));
        }
        public Waiter(UnityEngine.MonoBehaviour owner, Getter<bool> canContinue, Action onComplete)
        {
            this.owner = owner;
            current = null;
    ...
    {
        public static Lerper Lerp(this UnityEngine.MonoBehaviour owner, Getter<float> getter, Action<float> setter, float targetValue, float duration)
        {
            return new Lerper(owner, getter, setter, duration, targetValue);
        }
        
        public static Waiter Wait(this UnityEngine.MonoBehaviour owner, float delay, Action onComplete)
        {
            return new Waiter(owner, delay, onComplete);
        }
        
        public static Waiter WaitFor(this UnityEngine.MonoBehaviour owner, Getter<bool> canContinue, Action onComplete)
        {
