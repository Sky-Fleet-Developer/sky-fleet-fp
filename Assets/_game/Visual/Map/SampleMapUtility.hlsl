...
void WorldPositionToSlot(float3 world_position, out int slot)
{
    ...
    int2 slot_coord = floor(world_position.xz);
    slot = slot_coord.x * map_size + slot_coord.y;
}

void WorldPositionToSlot(float3 world_position, out float2 chunk_space_uv, out int slot)
{
    world_position.x = (world_position.x - position_to_chunk_matrix.z) * position_to_chunk_matrix.x;
    world_position.z = (world_position.z - position_to_chunk_matrix.w) * position_to_chunk_matrix.y;
    int2 slot_coord = floor(world_position.xz);
...
float2 SlotUvToChunkUvNoBorder(uint2 chunk, float2 uv)
{
    return chunk + uv;
}
